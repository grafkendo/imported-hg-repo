...
// WP Admin Functions
include ( get_stylesheet_directory() . '/inc/admin-functions.php' );

$fashion_stylist_uri = home_url() . '/wp-content/themes/fashion-stylist';

/**
 * After setup theme hook
 */
function remoter_theme_setup(){
    add_image_size( 'remoter-slider', 430, 600, true );
    add_image_size( 'remoter-blog-two', 410, 570, true );
}
add_action( 'after_setup_theme', 'remoter_theme_setup' );

if ( !function_exists( 'remoter_styles' ) ):
    function remoter_styles() {
    	$my_theme = wp_get_theme();
    	$version = $my_theme['Version'];
        
        wp_enqueue_style( 'blossom-fashion-style', trailingslashit( get_template_directory_uri() ) . 'style.css', array( 'animate' ) );
        
        wp_enqueue_style( 'remoter', get_stylesheet_directory_uri() . '/style.css', array( 'blossom-fashion-style' ), $version );
        
        wp_enqueue_script( 'remoter', get_stylesheet_directory_uri() . '/js/custom.js', array('jquery'), $version, true );
        
        $array = array( 
            'rtl'       => is_rtl(),
        ); 
        wp_localize_script( 'remoter', 'remoter_data', $array );
    }
endif;
add_action( 'wp_enqueue_scripts', 'remoter_styles', 10 );
...
function remoter_customizer_register( $wp_customize ) {
    global $fashion_stylist_uri;

    $wp_customize->get_control( 'slider_animation' )->active_callback   = 'remoter_slider_active_cb';
    
    $wp_customize->add_section( 'theme_info', array(
        'title'       => __( 'Demo & Documentation' , 'remoter' ),
        'priority'    => 6,
    ) );
    ...
    $wp_customize->add_control( 
        new Blossom_Fashion_Typography_Control( 
            ...
            'site_title_font', 
            array(
                'label'       => __( 'Site Title Font', 'remoter' ),
                'description' => __( 'Site title and tagline font.', 'remoter' ),
    ...
    $wp_customize->add_control(
        new Blossom_Fashion_Slider_Control( 
            ...
            array(
                'section'     => 'title_tagline',
                'label'       => __( 'Site Title Font Size', 'remoter' ),
                'description' => __( 'Change the font size of your site title.', 'remoter' ),
                'priority'    => 65,
                'choices'     => array(
                    'min'   => 10,
                    'max'   => 200,
    ...
    $wp_customize->add_control( 
        new WP_Customize_Color_Control( 
            ...
            'primary_color', 
            array(
                'label'       => __( 'Primary Color', 'remoter' ),
                'description' => __( 'Primary color of the theme.', 'remoter' ),
                'section'     => 'colors',
                'priority'    => 5,                
            )
        )
    ...
    $wp_customize->add_panel(
        'layout_settings',
        array(
            'title'    => __( 'Layout Settings', 'remoter' ),
            'priority' => 55,
        )
    );
    ...
    $wp_customize->add_section(
        'header_layout',
        array(
            'title'    => __( 'Header Layout', 'remoter' ),
    ...
    $wp_customize->add_control(
        new Blossom_Fashion_Radio_Image_Control(
            ...
            array(
                ...
                'label'       => __( 'Header Layout', 'remoter' ),
                'description' => __( 'This is the layout for header.', 'remoter' ),
                'choices'     => array(                 
                    'header-one'   => $fashion_stylist_uri . '/images/header/header-one.png',
                    'header-sec'   => $fashion_stylist_uri . '/images/header/header-two.png',
                )
            )
        )
    );
    ...
    $wp_customize->add_section(
        'blog_layout',
        array(
            'title'    => __( 'Home Page Layout', 'remoter' ),
            'panel'    => 'layout_settings',
            'priority' => 10,
        )
    );
    ...
    $wp_customize->add_control(
        new Blossom_Fashion_Radio_Image_Control(
            ...
            array(
                'section'     => 'blog_layout',
                'label'       => __( 'Home Page Layout', 'remoter' ),
                'description' => __( 'This is the layout for blog index page.', 'remoter' ),
                'choices'     => array(                 
                    'home-one'   => $fashion_stylist_uri . '/images/home/one-right.jpg',
                    'home-two'   => $fashion_stylist_uri . '/images/home/two-right.jpg',
                )
            )
        )
    );
    ...
    $wp_customize->add_section(
        ...
        array(
            'title'    => __( 'Slider Layout', 'remoter' ),
            'priority' => 20,
            'panel'    => 'layout_settings',
        )
    );
    ...
    $wp_customize->add_control(
        new Blossom_Fashion_Radio_Image_Control(
            ...
            array(
                ...
                'label'       => __( 'Slider Layout', 'remoter' ),
                'description' => __( 'Choose the layout of the slider for your site.', 'remoter' ),
                'choices'     => array(
                    'one'   => $fashion_stylist_uri . '/images/slider/one.jpg',
                    'two'   => $fashion_stylist_uri . '/images/slider/two.jpg',
                )
            )
        )
    );
...
add_action( 'customize_register', 'remoter_customizer_register', 40 );
...
function blossom_fashion_banner(){
    ...
    $image_size     = ( $slider_layout == 'two' ) ? 'remoter-slider' : 'blossom-fashion-slider';    
    $fallback_image = ( $slider_layout == 'two' ) ? $fashion_stylist_uri . '/images/fashion-stylist-slider.jpg' : get_template_directory_uri() . '/images/no-img_1920_760.jpg';    
...
*/
function remoter_slider_active_cb( $control ){
...
function blossom_fashion_category() {
    ...
    if ( 'post' === get_post_type() ) {
        ...
        $categories_list = get_the_category_list( esc_html__( ' ', 'remoter' ) );
        if ( $categories_list ) {
