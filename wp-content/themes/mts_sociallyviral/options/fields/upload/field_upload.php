...
class NHP_Options_upload extends NHP_Options{	
	...
	*/
	function render(){
		...
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload button-secondary"'.$upload.' rel-id="'.$this->field['id'].'">'.__('Browse', 'sociallyviral' ).'</a>';
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload-remove"'.$remove.' rel-id="'.$this->field['id'].'">'.__('Remove Upload', 'sociallyviral' ).'</a>';
	...
	*/
	function enqueue(){
		...
		wp_localize_script('nhp-opts-field-upload-js', 'nhp_upload', array(
			...
			'title' => __( 'Select Image', 'sociallyviral' ),
			'buttonText' => __( 'Select Image', 'sociallyviral' )
		));
