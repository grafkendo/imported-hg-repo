	...
	class mtscontact {
		...
		public function ajax_mtscontact() {
			if ($this->validate()) {
				if ($this->send_mail()) {
					...
					$this->errors['sendmail'] = __('An error occurred. Please contact site administrator.', 'sociallyviral' );
		...
		private function validate() {
			...
			if (!check_ajax_referer( 'mtscontact', 'mtscontact_nonce', false )) {
				$this->errors['nonce'] = __('Please try again.', 'sociallyviral' );
			}
			
			// check honeypot // must be empty
			if (!empty($_POST['mtscontact_captcha'])) {
				$this->errors['captcha'] = __('Please try again.', 'sociallyviral' );
			}
			
			// name field
			$name = trim(str_replace(array("\n", "\r", "<", ">"), '', strip_tags($_POST['mtscontact_name'])));
			if (empty($name)) {
				$this->errors['name'] = __('Please enter your name.', 'sociallyviral' );
			}
			
			// email field
			$useremail = trim($_POST['mtscontact_email']);
			if (!is_email($useremail)) {
				$this->errors['email'] = __('Please enter a valid email address.', 'sociallyviral' );
			}
			
			// message field
			$message = strip_tags($_POST['mtscontact_message']);
			if (empty($message)) {
				$this->errors['message'] = __('Please enter a message.', 'sociallyviral' );
			}
		...
		private function send_mail() {
			...
			$email_subject = __('Contact Form Message from', 'sociallyviral' ).' '.get_bloginfo('name');
			$email_message = __('Name:', 'sociallyviral' ).' '.$this->userinput['name']."\n\n".
							 __('Email:', 'sociallyviral' ).' '.$this->userinput['email']."\n\n".
							 __('Message:', 'sociallyviral' ).' '.$this->userinput['message'];
		...
		public function get_form() {
			...
			if (!$this->success) {
				...
				<label for="mtscontact_name">'.__('Name', 'sociallyviral' ).'</label>
				<input type="text" name="mtscontact_name" value="'.esc_attr($this->userinput['name']).'" id="mtscontact_name" />
				
				<label for="mtscontact_email">'.__('Email', 'sociallyviral' ).'</label>
				<input type="text" name="mtscontact_email" value="'.esc_attr($this->userinput['email']).'" id="mtscontact_email" />
				
				<label for="mtscontact_message">'.__('Message', 'sociallyviral' ).'</label>
				<textarea name="mtscontact_message" id="mtscontact_message">'.esc_textarea($this->userinput['message']).'</textarea>
				
				<input type="submit" value="'.__('Send', 'sociallyviral' ).'" id="mtscontact_submit" />
			</form>';
			}
			$return .= '<div id="mtscontact_success"'.($this->success ? '' : ' style="display: none;"').'>'.__('Your message has been sent.', 'sociallyviral' ).'</div>';
			return $return;
		}
