...
<div id="plugins-panel" class="panel-left visible">
	<h4><?php _e( 'Recommended Plugins', 'blossom-fashion' ); ?></h4>

	<p><?php printf( __( 'Below is a list of recommended plugins to install that will help you get the most out of %1$s. Although each plugin is optional, it is recommended that you at least install the BlossomThemes Toolkit, BlossomThemes Email Newsletter & BlossomThemes Instagram Feed to create a website similar to the %1$s demo.', 'blossom-fashion' ), $theme['Name'] ); ?></p>
	...
	if( !empty( $free_plugins ) ) { ?>
		<h4 class="recomplug-title"><?php echo esc_html__( 'Free Plugins', 'blossom-fashion' ); ?></h4>
		<p><?php echo esc_html__( 'These Free Plugins might be handy for you.', 'blossom-fashion' ); ?></p>
		...
		foreach( $free_plugins as $plugin ) {
			...
			<div class="recom-plugin-wrap">
				...
				<div class="plugin-title-install clearfix">
					...
					echo Blossom_Fashion_Getting_Start_Page_Plugin_Helper::instance()->get_button_html( $plugin['slug'] );
					echo '</div>';
					?>
				</div>
			</div>
